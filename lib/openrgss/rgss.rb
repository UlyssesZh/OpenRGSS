...
module RGSS
	...
	# Outputs the arguments to the message box. If a non-string object has been
	# supplied as an argument, it will be converted into a string with to_s and output.
	#
	# Returns nil.
	#
	...
	def msgbox *args
		if RUBY_PLATFORM['mingw'] or RUBY_PLATFORM['mswin']
			...
			@@messagebox.call [0, args.map(&:to_s).join(?\n), RGSS.title, 0].pack('L!ppL!').unpack('L!*')
		else
			# TODO
			puts args
		end
	...
	module Drawable
		...
		def > v
			return false if !@viewport && v.viewport
			if v.viewport
				return false if @viewport.z < v.viewport.z
				return false if @viewport.z == v.viewport.z && @viewport.created_at < v.viewport.created_at
			end
			return false if @z < v.z
			return false if @z == v.z && @y < v.y
			return false if @z == v.z && @y == v.y && @created_at < v.created_at
			true
		end

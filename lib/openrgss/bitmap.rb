...
class Bitmap
	...
	def fill_rect x, y, width = nil, height = nil, color = nil
		...
		@entity.fill_rect x, y, width, height, @entity.map_rgba(*color)
	end
	...
	def gradient_fill_rect x, y, width, height = false, color1 = nil, color2 = nil, vertical = false
		if x.is_a? Rect
			color1, color2, vertical = y, width, height
			x, y, width, height = *x
		end
		if vertical
			height.times do |i|
				@entity.fill_rect x, y + i, width, 1, @entity.map_rgba(
						*color1.zip_map(color2) { |c1, c2| c1 + (c2 - c1) * i / height }
				)
			end
		else
			width.times do |i|
				@entity.fill_rect x + i, y, 1, height, @entity.map_rgba(
						*color1.zip_map(color2) { |c1, c2| c1 + (c2 - c1) * i / width }
				)
			end
		end
	end
	...
	def clear_rect x, y = nil, width = nil, height = nil
		x, y, width, height = *x if x.is_a? Rect
	...
	def set_pixel x, y, color
		@entity.put_pixel x, y, @entity.map_rgba(*color)
	end

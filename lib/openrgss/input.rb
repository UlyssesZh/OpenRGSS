...
module Input
	...
	class << self
		...
		# Determines whether the button corresponding to the symbol sym is currently
		# being pressed.
		#
		...
		def dir8
			if @status[:DOWN] && @status[:LEFT]
				1
			elsif @status[:DOWN] && @status[:RIGHT]
				3
			elsif @status[:DOWN]
				2
			elsif @status[:UP] && @status[:LEFT]
				7
			elsif @status[:UP] && @status[:RIGHT]
				9
			elsif @status[:UP]
				8
			elsif @status[:LEFT]
